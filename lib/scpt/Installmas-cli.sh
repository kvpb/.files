...
#{
	local latestrelease=$(curl -L -s -H 'Accept: application/json' https://github.com/mas-cli/mas/releases/latest)
	local latestversion=$(echo $latestrelease | sed -e 's/.*"tag_name":"\([^"]*\)".*/\1/') # The releases are returned in the format {"id":3622206,"tag_name":"hello-1.0.0.11",...}, we have to extract the tag_name.
	local latestreleaseURL="https://github.com/mas-cli/mas/releases/download/${latestversion}/mas-cli.zip"
#}
